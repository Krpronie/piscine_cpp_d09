...
CommunicationAPI::CommunicationAPI(std::istream &input, std::ostream &output): _usersOffset(0),_input(input),_output(output)
{
    if (!_input.good())
...
void
CommunicationAPI::sendMessage(std::string const &userName,std::string const &message) const
{
    if (!hasUser(userName))
...
void
CommunicationAPI::receiveMessage(std::string const &userName,std::string &message) const
{
    if (!hasUser(userName))
        throw std::invalid_argument("User " + userName + " not found.");
    if (_missionName.size() == 0)
